...
- [x] SEO pass via the `seo` skill (vendored at `.claude/skills/seo/`, MIT, from github.com/aevans-eng/seo-skill): `app/robots.ts`, `app/sitemap.ts`, canonical URL, Person + WebSite JSON-LD on the homepage, shared `SITE_URL` constant in `content.ts`. Remaining manual step: verify the domain in Google Search Console and submit the sitemap.
- [ ] Portrait: the intro has a photo slot that renders once `public/adithya.jpg` exists (build-time `fs.existsSync` guard, so the site builds and deploys cleanly without it). The photo itself could not be transferred from chat into the sandbox; add the file via GitHub upload or a local session.
